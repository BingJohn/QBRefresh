...
/// 刷新控件的3种状态
private enum QBRefreshStatus: Int {
...
class QBHeaderRefreshControl: UIView {
    ...
    var isRefreshing:Bool {
        get {
            return currentStatus == .Refreshing
        }
    }
    
    /// 记录上一次的状态
    private var oldStatus: QBRefreshStatus = .Normal
    
    /// 刷新控件的当前刷新状态,默认是 Normal:箭头朝下, 下拉刷新
    private var currentStatus: QBRefreshStatus = .Normal {
        didSet {
            // 状态改变后来改变UI
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if scrollView.isDragging {
            ...
            if currentStatus == .PullToRefresh && offsetY > normal2pullingOffsetY {   // -64. -scrollView.contentInset.top - PulldownRefreshControlHeight
                print("切换到Normal")
                currentStatus = .Normal
            } else if currentStatus == .Normal && offsetY < normal2pullingOffsetY {
                print("切换到PullToRefresh")
...
class QBFooterRefreshControl: UIView {
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if scrollView.isDragging {
            if currentStatus == .PullToRefresh && offsetY < normal2pullingOffsetY {
                print("切换到Normal")
                currentStatus = .Normal
            } else if currentStatus == .Normal && offsetY > normal2pullingOffsetY {
                print("切换到PullToRefresh")
    ...
    // MARK: - 懒加载
    /// 箭头
    private lazy var arrowView: UIImageView = UIImageView(image: UIImage(named: "tableview_pull_up_refresh"))
    /// 风火轮
    private lazy var fhlView: UIImageView = UIImageView(image: UIImage(named: "tableview_loading"))
